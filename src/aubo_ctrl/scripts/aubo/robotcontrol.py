...
print basedir
liblog4cplus = cdll.LoadLibrary(basedir + '/liblog4cplus-1.2.so.5')
libconfig = cdll.LoadLibrary(basedir + '/libconfig.so')
aa = cdll.LoadLibrary(basedir + '/libprotobuf.so.9')
...
class Auboi5Robot:
    ...
    def move_to_target_in_cartesian(self, pos, rpy_xyz):
        ...
        * NOTES:
        ...
        if self.rshd >= 0 and self.connected:
            ...
            result = libpyauboi5.move_line(self.rshd, ik_result["joint"])
            if result != RobotErrorType.RobotError_SUCC:
    ...
    def movepath(self,points):
        ...
        # 设置机械臂末端最大线加速度(m/s)
        self.set_end_max_line_acc(0.5)

        # 获取机械臂末端最大线加速度(m/s)
        self.set_end_max_line_velc(0.2)

        # 度 -> 弧度
        rpy_xyz = [i / 180.0 * pi for i in [175, 0, -85]]
        # 欧拉角转四元数
        ori = libpyauboi5.rpy_to_quaternion(self.rshd, rpy_xyz)
        ...
        ik_result = libpyauboi5.inverse_kin(self.rshd, joint_radian['joint'], point1, ori)
        logging.info("ik_result111====>{0}".format(ik_result))
        self.move_joint(ik_result["joint"])


        # 清除所有已经设置的全局路点
        self.remove_all_waypoint()
        ...
        for i in range(0,10,3):
            ...
            # joint_radian = [0, 0, 0, 0, 0, 0]
            # logger.info(joint_radian)
            logger.info("point:{0}".format(point))
            ik_result = libpyauboi5.inverse_kin(self.rshd, joint_radian['joint'], point, ori)
            ...
            self.add_waypoint(ik_result["joint"])

        # # 设置圆运动圈数
        # robot.set_circular_loop_times(3)

        self.set_blend_radius(0.001)

        self.move_track(RobotMoveTrackType.CARTESIAN_MOVEP)
