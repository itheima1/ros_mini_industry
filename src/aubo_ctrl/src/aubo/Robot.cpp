...
//    std::cout << " rx: " << rpy.rx * RA2DE << " ry: " << rpy.ry * RA2DE<< " rz: " << rpy.rz * RA2DE << std::endl;
    ...
    return robotService.robotServiceLineMove(angles, IsBolck);
}
 ...
 * @return
 */
int Robot::moveL2(double jointAngle[aubo_robot_namespace::ARM_DOF], bool IsBolck) {
    SetGlobalMoveEndMaxParams();
...
void Robot::SetGlobalMoveJointMaxParams() {
    ...
    jointMaxVelc.jointPara[1] = maxJointVelc / 180.0 * M_PI;
    jointMaxVelc.jointPara[2] = maxJointVelc / 180.0 * M_PI;
    jointMaxVelc.jointPara[3] = maxJointVelc / 180.0 * M_PI;
    jointMaxVelc.jointPara[4] = maxJointVelc / 180.0 * M_PI;
    jointMaxVelc.jointPara[5] = maxJointVelc / 180.0 * M_PI;   //接口要求单位是弧度
//设置最大加速度
